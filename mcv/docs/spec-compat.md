...
| `io.kserve.km/split-cache-capture` | `"true"` when `cache-mounts` is set (primary vLLM root plus extra `--source` trees); MCV extract uses capture-root layout under `--dir` |
